...
if [[ $1 == 'squashfs' ]]; then
    ...
    time srun singularity exec $SIF bash -c 'python scripts/lmdb/convert_to_lmdb.py'
elif [[ $1 == 'hdf5' ]]; then
    mkdir -p data-formats/hdf5/
    time srun singularity exec $SIF bash -c 'python scripts/hdf5/convert_to_hdf5.py'
fi
