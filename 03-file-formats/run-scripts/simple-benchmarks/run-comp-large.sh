...
if [[ $1 == 'squashfs' ]]; then
    ...
    srun singularity exec -B $SQUASH:/train_images:image-src=$IMAGES $SIF bash -c 'python run-scripts/simple-benchmarks/compare-dataset-large.py -n 7 -ff "squashfs" -N 200000'
elif [[ $1 == 'lmdb' ]]; then
    srun singularity exec  $SIF bash -c 'python run-scripts/simple-benchmarks/compare-dataset-large.py -n 7 -ff "lmdb" -N 200000'
elif [[ $1 == 'hdf5' ]]; then
